...
\begin{definition}
  ...
  every polynomial over $F$ has a root in $F$.
\end{definition}
...
\begin{example}
  \begin{enumerate}
    ...
    \item For every prime number $p$, the polynomial $q(x) = x^p + 1$ does not have a root 
    in $\bb{F}_p$. Thus, $\bb{F}_p$ is not algebraically closed.
  \end{enumerate}
\end{example}
...
\begin{definition}
  ...
  $\{b_1, \ldots, b_n\} \subseteq B$, there is no polynomial $p(x_1, \ldots, x_n)$ over 
  $K$ such that 
  \[
    p(b_1, \ldots, b_n) = 0.
  \]
